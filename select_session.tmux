...
tmux_sort_order="@fzf_session_switch_sort-order"
...
set_switch_session_bindings() {
    local bind_key preview fzf_window_position fzf_preview_position min_preview_lines hide_current_session sort_order
    ...
    sort_order="$(get_tmux_option "${tmux_sort_order}" "${default_sort_order}")"

    tmux bind-key "${bind_key}" run-shell \
        "'${CURRENT_DIR}/select_session.sh' '${preview}' '${fzf_window_position}' '${fzf_preview_position}' '${min_preview_lines}' '${hide_current_session}' '${sort_order}'"
}
